        ...
        <>
          <Grid container direction="row" justify="space-between" spacing={3}>
            <Grid item lg={4} md={12}>
              ...
              >
                <Box className={classes.formControlBox}>
                  ...
                  <Select
                    ...
                    value={
                      filteredObjectiveId !== null ? filteredObjectiveId : ""
                    }
                    onChange={(e) => {
                      const val = e.target.value;
                      if (isValidNumber(val.toString())) {
                        console.log("FASZ XD");
                        setFilteredObjectiveId(
                    ...
                    {fetchedObjectives.map((objective) => {
                      return (
                        <option key={objective.id} value={objective.id}>
                          {`${
                            objectiveTypeData[objective.objectiveType]
                              .shortDisplayName
                          } > ${objective.title}`}
                        </option>
                      );
              ...
              >
                <Table size="small" className={classes.objectiveTable}>
                  ...
                  <TableBody>
                    {fetchedObjectives &&
                      fetchedObjectives.map(
                        (objective: Objective, index: number) => {
                          return (
                            <TableRow
                              key={index}
                              onClick={() =>
                                setFilteredObjectiveId(objective.id)
                              }
                              hover
                            ...
                            >
                              ...
                              <TableCell>
                                {objective.submittable ? "Beadható" : "Lejárt"}
                              </TableCell>
                              <TableCell>
            ...
            <Grid item lg={8} md={12}>
              ...
              <SubmissionsPanel
                filteredObjectiveId={filteredObjectiveId}
