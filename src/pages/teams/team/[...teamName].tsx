...
const Index: NextPage = () => {
  ...
  const isViewedByMemberOrLeaderOfTeam =
    usedTeamInfo.data &&
    currentUser.isMemberOrLeaderOfTeam(usedTeamInfo.data.id);
  const isViewedByLeaderOfTeam =
    usedTeamInfo.data && currentUser.isLeaderOfTeam(usedTeamInfo.data.id);
    ...
    callJsonEndpoint({
      ...
      .catch((err) => {
        if (err instanceof ApiErrorDescriptorException) {
          if (
            teamLifecycle_THERE_IS_NO_OTHER_LEADER.is(err.apiErrorDescriptor)
          ) {
            const confirmText =
  ...
  function submitCancelApplication() {
    callJsonEndpoint({
      ...
      .then(() => {
        EventBus.notifySuccess(
          "You're now free to join or create other teams",
  ...
  function submitApply() {
    ...
    callJsonEndpoint({
      ...
      .then(() => {
        EventBus.notifySuccess(
          "The team leaders should review it soon",
  ...
  return (
    <>
      <Head>
        <title>
          {usedTeamInfo.data ? usedTeamInfo.data.name + " " : "Qpa "} Team
        </title>
      </Head>
      <Paper>
        <NotTeamMemberBanner />
        ...
        {usedTeamInfo.data && (
          <Grid container direction="row" alignItems="center">
            ...
            {currentUser.getUserInfo()?.teamRole === TeamRole.NOTHING &&
              !usedTeamInfo.data.archived && (
            ...
            <List component="nav">
              {usedTeamApplicants.data &&
                usedTeamApplicants.data.map((applicant) => {
                  const basicDisplayName =
                    UserNameFormatter.getBasicDisplayName(applicant);
                  return (
                    <ListItem key={applicant.userId}>
                      <Link
                        href={`/users/user/${UserNameFormatter.getUrlName(
                          applicant
                        )}?id=${applicant.userId}`}
                      >
                        <Grid container direction="row" alignItems="center">
                      ...
                      <ListItemSecondaryAction>
                        <Tooltip title="Engedélyez">
                          <IconButton
                            onClick={() =>
                              submitApproveApplication(applicant.userId)
                            }
                          >
                            <PersonAddIcon />
                          </IconButton>
                        ...
                        <Tooltip title="Elutasít">
                          <IconButton
                            onClick={() =>
                              submitDeclineApplication(applicant.userId)
                            }
                          >
          ...
          <List component="nav">
            {usedTeamMembers.data &&
              usedTeamMembers.data.map((member, index) => {
                const basicDisplayName =
                  UserNameFormatter.getBasicDisplayName(member);
                return (
                  <ListItem key={member.userId}>
                    ...
                    {isViewedByLeaderOfTeam &&
                      member.userId !== currentUser.getUserInfo()?.userId && (
                        <ListItemSecondaryAction>
                          <Tooltip title="Kirugás">
                            <IconButton
                              onClick={() => submitKick(member.userId)}
                            >
                              <MeetingRoomIcon />
                            </IconButton>
                          </Tooltip>
                          {member.teamRole === TeamRole.MEMBER && (
                            <Tooltip title="Kinevezés csapatkapitánnyá">
                              <IconButton
                                onClick={() =>
                                  submitGiveLeaderRights(member.userId)
                                }
                              >
                                <ArrowUpwardIcon />
                              </IconButton>
                          ...
                          {member.teamRole === TeamRole.LEADER && (
                            <Tooltip title="Inasba rakni">
                              <IconButton
                                onClick={() =>
                                  submitTakeAwayLeaderRights(member.userId)
                                }
                              >
                                <ArrowDownwardIcon />
                              </IconButton>
